...
os.environ['FOR_DISABLE_CONSOLE_CTRL_HANDLER'] = '1'  # 避免scipy引发的键盘中断事件（ctrl+c）占用
import platform
import sys
import time
import natsort
...
from alphapose.utils.writer import DataDealer
...
# parser.add_argument('--checkpoint', type=str, default='../weights/fast_res50_256x192.pth',
#                     help='checkpoint file name')
parser.add_argument('--cfg', type=str, default='configs/halpe_136/resnet/256x192_res50_lr1e-3_2x-regression.yaml',
                    help='experiment configure file name')
parser.add_argument('--checkpoint', type=str, default='weights/halpe136_fast_res50_256x192.pth',
                    help='checkpoint file name')
...
def print_finish_info():
    ...
    if (args.save_img or args.save_video) and not args.vis_fast:
        ...
        print(
            '===========================> If this step takes too long, you can enable the --vis_fast flag to use fast '
            'rendering (real-time).')
...
if __name__ == "__main__":
    ...
    if args.save_video and mode != 'image':
        ...
        writer = DataDealer(cfg, args, save_video=True, video_save_opt=video_save_opt, queueSize=queueSize).start()
    else:
        writer = DataDealer(cfg, args, save_video=False, queueSize=queueSize).start()
    ...
    try:
        for i in im_names_desc:
            ...
            with torch.no_grad():
                ...
                # igs = inps.cpu().detach().permute(0, 2, 3, 1).numpy()
                # igs = (igs * 0.5 + 0.5)
                #
                # for idx in range(igs.shape[0]):
                #     ig = igs[idx]
                #     cv2.imshow(str(idx), ig[:, :, ::-1])
                # cv2.waitKey(15)
